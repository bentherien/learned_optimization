# coding=utf-8
# AdaFac + AdaFisher — Version: FacFisher
#
# Key idea: compare AdaFac's own curvature estimates (fac_g, fac_mom_mult)
# with the Fisher curvature via outer products and log-alignment.
# Tells the MLP whether the two independent curvature families agree.
...
class AdaFisherFacFisherMLPLOptState:
    params:               Any
    state:                Any
...
@gin.configurable
class AdaFisherFacFisherMLPLOpt(lopt_base.LearnedOptimizer):
    """
    AdaFac MLP learned optimizer — Version: FacFisher.

    Fisher features appended:
        h_d, s_d, f_tilde, rsqrt_f          standard Fisher context   (*, n_f each)
        g * rsqrt_f                           preconditioned gradient  (*, n_f)
        facg_fisher  = fac_g ⊗ rsqrt_f       AdaFac grad × Fisher     (*, n_fac*n_f)
        fmm_fisher   = fac_mom_mult ⊗ rsqrt_f double-preconditioned mom (*, n_fac*n_f)
        log_facg_rel = log|fac_g| − mean(log f_tilde)  alignment diag  (*, n_fac)
    """
    ...
    def _mod(self, global_feat, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row, fac_vec_v,
        ...
        facg_exp     = fac_g[..., :, None]                               # (*, n_fac, 1)
        facg_fisher  = (facg_exp * rsqrt_exp).reshape(p.shape + (n_fac * n_f,))
        inps.append(facg_fisher)

        fmm_exp      = fac_mom_mult[..., :, None]                        # (*, n_fac, 1)
        fmm_fisher   = (fmm_exp * rsqrt_exp).reshape(p.shape + (n_fac * n_f,))
        inps.append(fmm_fisher)

        log_facg_rel = (jnp.log(jnp.abs(fac_g) + 1e-12)
                        - jnp.mean(jnp.log(f_tilde + 1e-12), axis=-1, keepdims=True))
        inps.append(log_facg_rel)
        ...
        summary.summary("adafac_fisher_facfisher/avg_step_size", avg_step)
        summary.summary("adafac_fisher_facfisher/avg_step_size_hist", avg_step, aggregation="collect")
        summary.summary("adafac_fisher_facfisher/f_tilde/mean", jnp.mean(f_tilde))
        summary.summary("adafac_fisher_facfisher/lambda", lam)
        return new_p
    ...
    def opt_fn(self, theta, is_training=False):
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def init(self, params, model_state=None, num_steps=None, key=None):
                ...
                return AdaFisherFacFisherMLPLOptState(
            ...
            def update(self, opt_state, grad, loss, model_state=None, is_valid=False, key=None):
                ...
                next_opt_state = AdaFisherFacFisherMLPLOptState(
