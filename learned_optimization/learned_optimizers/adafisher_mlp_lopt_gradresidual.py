# coding=utf-8
# AdaFac + AdaFisher — Version: GradResidual
#
# Key idea: compare the *instantaneous* batch curvature (computed from the
# current gradient) against the EMA Fisher estimates h_d / s_d.
# The residual encodes curvature "surprise" — whether we are in a spike
# (high-curvature step) or a flat region relative to recent history.
...
class AdaFisherGradResidualMLPLOptState:
    params:               Any
    state:                Any
...
@gin.configurable
class AdaFisherGradResidualMLPLOpt(lopt_base.LearnedOptimizer):
    """
    AdaFac MLP learned optimizer — Version: GradResidual.

    Fisher features appended:
        delta_h = log(h_now) − log(h_d_ema)   log-residual col-curvature  (*, n_f)
        delta_s = log(s_now) − log(s_d_ema)   log-residual row-curvature  (*, n_f)
        h_ratio = h_now / h_d_ema              multiplicative surprise     (*, n_f)
        s_ratio = s_now / s_d_ema                                          (*, n_f)
        g * rsqrt_f_now                        instant-Fisher precond grad (*, n_f)
        g * rsqrt_f_ema                        EMA-Fisher precond grad     (*, n_f)
        h_d, s_d                               EMA context                 (*, n_f each)
    """
    ...
    def _mod(self, global_feat, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row, fac_vec_v,
        ...
        # Instantaneous curvature from current gradient
        h_now = _compute_h_d(g, n_f)   # (*, n_f)
        s_now = _compute_s_d(g, n_f)   # (*, n_f)

        # Log residuals (signed curvature surprise in log space)
        delta_h = jnp.log(h_now + 1e-12) - jnp.log(h_d + 1e-12)   # (*, n_f)
        delta_s = jnp.log(s_now + 1e-12) - jnp.log(s_d + 1e-12)   # (*, n_f)

        # Multiplicative surprise (clipped to avoid extreme values at init)
        h_ratio = jnp.clip(h_now / (h_d + 1e-12), 0.0, 1e4)        # (*, n_f)
        s_ratio = jnp.clip(s_now / (s_d + 1e-12), 0.0, 1e4)        # (*, n_f)

        # Fisher preconditioner from EMA
        f_tilde_ema = jnp.maximum(s_d * h_d + lam, 1e-12)
        rsqrt_f_ema = lax.rsqrt(f_tilde_ema + 1e-12)

        # Fisher preconditioner from instantaneous curvature
        f_tilde_now = jnp.maximum(s_now * h_now + lam, 1e-12)
        rsqrt_f_now = lax.rsqrt(f_tilde_now + 1e-12)

        g_exp = jnp.expand_dims(g, -1)
        inps.append(delta_h); inps.append(delta_s)
        inps.append(h_ratio); inps.append(s_ratio)
        inps.append(g_exp * rsqrt_f_now)   # instant-Fisher preconditioned gradient
        inps.append(g_exp * rsqrt_f_ema)   # EMA-Fisher preconditioned gradient
        inps.append(h_d); inps.append(s_d)
        ...
        summary.summary("adafac_fisher_gradresidual/avg_step_size", avg_step)
        summary.summary("adafac_fisher_gradresidual/avg_step_size_hist", avg_step, aggregation="collect")
        summary.summary("adafac_fisher_gradresidual/f_tilde_ema/mean", jnp.mean(f_tilde_ema))
        summary.summary("adafac_fisher_gradresidual/lambda", lam)
        return new_p
    ...
    def opt_fn(self, theta, is_training=False):
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def init(self, params, model_state=None, num_steps=None, key=None):
                ...
                return AdaFisherGradResidualMLPLOptState(
            ...
            def update(self, opt_state, grad, loss, model_state=None, is_valid=False, key=None):
                ...
                next_opt_state = AdaFisherGradResidualMLPLOptState(
