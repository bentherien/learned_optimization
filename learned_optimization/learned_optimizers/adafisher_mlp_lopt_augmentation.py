# coding=utf-8
# AdaFac + AdaFisher augmentation — Version: Augmentation
#
# Feature block: h_d, s_d, f_tilde, rsqrt_f, preconditioned gradient,
# and mean-momentum preconditioned by Fisher.
...
class AdaFisherAugMLPLOptState:
    params:               Any
    state:                Any
...
@gin.configurable
class AdaFisherAugMLPLOpt(lopt_base.LearnedOptimizer):
    """
    AdaFac MLP learned optimizer — Version: Augmentation.
    ...
    Fisher features appended to AdaFac feature vector:
        ...
        m_mean*rsqrt_f mean-momentum preconditioned by Fisher  (*, n_f)
    """
    ...
    def _mod(self, global_feat,
        ...
        inps.append(m_mean * rsqrt_f)                      # (*, n_f)
        ...
        summary.summary("adafac_fisher_aug/avg_step_size", avg_step)
        summary.summary("adafac_fisher_aug/avg_step_size_hist", avg_step, aggregation="collect")
        summary.summary("adafac_fisher_aug/direction/mean_abs",  jnp.mean(jnp.abs(direction)))
        summary.summary("adafac_fisher_aug/magnitude/mean_abs",  jnp.mean(jnp.abs(magnitude)))
        summary.summary("adafac_fisher_aug/magnitude/mean",      jnp.mean(magnitude))
        summary.summary("adafac_fisher_aug/grad/mean_abs",       jnp.mean(jnp.abs(g)))
        summary.summary("adafac_fisher_aug/f_tilde/mean",        jnp.mean(f_tilde))
        summary.summary("adafac_fisher_aug/lambda",              lam)
    ...
    def opt_fn(self,
        ...
        class _Opt(opt_base.Optimizer):
            ...
            ) -> AdaFisherAugMLPLOptState:
                if num_steps is None:
                    raise ValueError("AdaFisherAugMLPLOpt requires num_steps at init time.")

                mom_roll, rms_roll, fac_vec_roll = self._get_rolling()
                n_f = len(parent._initial_fisher_decays)

                return AdaFisherAugMLPLOptState(
                    params=params,
                    state=model_state,
            ...
            def update(
                    self,
                    opt_state:   AdaFisherAugMLPLOptState,
                ...
                next_opt_state = AdaFisherAugMLPLOptState(
                    params=next_params,
                    state=model_state,
