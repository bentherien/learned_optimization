# coding=utf-8
# AdaFac + AdaFisher — Version: ParamFisher
#
# Key idea: combine the parameter value p with Fisher curvature to produce
# geometry-aware features.  p * sqrt(F) is analogous to the Newton decrement
# ||p||_F in the Fisher metric; p * rsqrt(F) is the Fisher-whitened parameter.
# These tell the MLP whether a parameter is large/small *relative to* the
# curvature at its location — information lost when p and F are concatenated
# independently as in all existing blocks.
...
class AdaFisherParamFisherMLPLOptState:
    params:               Any
    state:                Any
...
@gin.configurable
class AdaFisherParamFisherMLPLOpt(lopt_base.LearnedOptimizer):
    """
    AdaFac MLP learned optimizer — Version: ParamFisher.

    Fisher features appended:
        p * sqrt(f_tilde)                Newton-decrement-like            (*, n_f)
        p * rsqrt(f_tilde)               Fisher-whitened parameter        (*, n_f)
        p * g * rsqrt(f_tilde)           signed joint p-g Fisher feature  (*, n_f)
        log|p| − log(f_tilde)            log scale of p vs curvature      (*, n_f)
        h_d, s_d, f_tilde, rsqrt_f       standard Fisher context          (*, n_f each)
    ...
    def _mod(self, global_feat, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row, fac_vec_v,
        ...
        sqrt_f   = jnp.sqrt(f_tilde + 1e-12)

        p_exp = jnp.expand_dims(p, -1)   # (*, 1)
        g_exp = jnp.expand_dims(g, -1)   # (*, 1)

        inps.append(p_exp * sqrt_f)                                          # (*, n_f)
        inps.append(p_exp * rsqrt_f)                                         # (*, n_f)
        inps.append(p_exp * g_exp * rsqrt_f)                                 # (*, n_f)
        inps.append(jnp.log(jnp.abs(p_exp) + 1e-12) - jnp.log(f_tilde + 1e-12))  # (*, n_f)
        inps.append(h_d); inps.append(s_d); inps.append(f_tilde); inps.append(rsqrt_f)
        inps.append(g_exp * rsqrt_f)
        ...
        summary.summary("adafac_fisher_paramfisher/avg_step_size", avg_step)
        summary.summary("adafac_fisher_paramfisher/avg_step_size_hist", avg_step, aggregation="collect")
        summary.summary("adafac_fisher_paramfisher/f_tilde/mean", jnp.mean(f_tilde))
        summary.summary("adafac_fisher_paramfisher/lambda", lam)
        return new_p
    ...
    def opt_fn(self, theta, is_training=False):
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def init(self, params, model_state=None, num_steps=None, key=None):
                ...
                return AdaFisherParamFisherMLPLOptState(
            ...
            def update(self, opt_state, grad, loss, model_state=None, is_valid=False, key=None):
                ...
                next_opt_state = AdaFisherParamFisherMLPLOptState(
