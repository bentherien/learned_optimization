# coding=utf-8
# AdaFac + AdaFisher augmentation — Version: Fisher × RMS Cross-Feature
#
# Feature block: Fisher × RMS cross (outer product) + log-ratio + raw Fisher features.
...
class AdaFisherRMSCrossMLPLOptState:
    params:               Any
    state:                Any
...
@gin.configurable
class AdaFisherRMSCrossMLPLOpt(lopt_base.LearnedOptimizer):
    """
    AdaFac MLP learned optimizer — Version: Fisher × RMS Cross-Feature.

    Fisher features appended to AdaFac feature vector:
        rms_fisher_cross   rsqrt(rms) × rsqrt(f_tilde) outer product  (*, n_rms*n_f)
        log_ratio          log(f_tilde) − log(rms) outer product       (*, n_rms*n_f)
    ...
    def _mod(self, global_feat,
        ...
        # ── Fisher × RMS Cross-Feature block ────────────────────────────
        n_rms = rms.shape[-1]
        n_f   = h_d.shape[-1]
        ...
        summary.summary("adafac_fisher_rmscross/avg_step_size", avg_step)
        summary.summary("adafac_fisher_rmscross/avg_step_size_hist", avg_step, aggregation="collect")
        summary.summary("adafac_fisher_rmscross/direction/mean_abs",  jnp.mean(jnp.abs(direction)))
        summary.summary("adafac_fisher_rmscross/magnitude/mean_abs",  jnp.mean(jnp.abs(magnitude)))
        summary.summary("adafac_fisher_rmscross/magnitude/mean",      jnp.mean(magnitude))
        summary.summary("adafac_fisher_rmscross/grad/mean_abs",       jnp.mean(jnp.abs(g)))
        summary.summary("adafac_fisher_rmscross/f_tilde/mean",        jnp.mean(f_tilde))
        summary.summary("adafac_fisher_rmscross/lambda",              lam)
    ...
    def opt_fn(self,
        ...
        class _Opt(opt_base.Optimizer):
            ...
            ) -> AdaFisherRMSCrossMLPLOptState:
                if num_steps is None:
                    raise ValueError("AdaFisherRMSCrossMLPLOpt requires num_steps at init time.")

                mom_roll, rms_roll, fac_vec_roll = self._get_rolling()
                n_f = len(parent._initial_fisher_decays)

                return AdaFisherRMSCrossMLPLOptState(
                    params=params,
                    state=model_state,
            ...
            def update(
                    self,
                    opt_state:   AdaFisherRMSCrossMLPLOptState,
                ...
                next_opt_state = AdaFisherRMSCrossMLPLOptState(
                    params=next_params,
                    state=model_state,
