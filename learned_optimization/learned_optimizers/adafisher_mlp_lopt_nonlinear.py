# coding=utf-8
# AdaFac + AdaFisher — Version: Nonlinear
#
# Key idea: apply bounded nonlinear transformations to the Fisher-preconditioned
# gradient instead of using raw linear features.
#   tanh(g * rsqrt_f)       — bounded preconditioned gradient in [-1, 1]
#   (g * rsqrt_f)²          — Fisher-normalized gradient energy (always >= 0)
#   g² * rsqrt_f            — squared gradient weighted by Fisher precision
#   sign(g) * rsqrt_f       — curvature-weighted gradient direction
# These saturating and nonlinear representations may better condition the MLP
# than the raw linear features, especially in the presence of gradient spikes.
...
class AdaFisherNonlinearMLPLOptState:
    params:               Any
    state:                Any
...
@gin.configurable
class AdaFisherNonlinearMLPLOpt(lopt_base.LearnedOptimizer):
    """
    AdaFac MLP learned optimizer — Version: Nonlinear.

    Fisher features appended:
        prec_g = g * rsqrt_f                 linear preconditioned gradient   (*, n_f)
        tanh(prec_g)                          bounded in [-1,1]                (*, n_f)
        prec_g²  = (g * rsqrt_f)²            Fisher-normalized gradient energy (*, n_f)
        g² * rsqrt_f                          squared grad weighted by Fisher  (*, n_f)
        sign(g) * rsqrt_f                     curvature-weighted direction     (*, n_f)
        h_d, s_d, f_tilde, rsqrt_f            Fisher context                  (*, n_f each)
    """
    ...
    def _mod(self, global_feat, p, g, m, rms, fac_g, fac_vec_col, fac_vec_row, fac_vec_v,
        ...
        g_exp   = jnp.expand_dims(g, -1)                    # (*, 1)
        prec_g  = g_exp * rsqrt_f                            # (*, n_f)

        inps.append(prec_g)                                  # linear preconditioned gradient
        inps.append(jnp.tanh(prec_g))                        # bounded in [-1, 1]
        inps.append(jnp.square(prec_g))                      # Fisher-normalised gradient energy
        inps.append(jnp.square(g_exp) * rsqrt_f)             # g² * rsqrt_f
        inps.append(jnp.sign(g_exp) * rsqrt_f)               # curvature-weighted direction

        inps.append(h_d); inps.append(s_d); inps.append(f_tilde); inps.append(rsqrt_f)
        ...
        summary.summary("adafac_fisher_nonlinear/avg_step_size", avg_step)
        summary.summary("adafac_fisher_nonlinear/avg_step_size_hist", avg_step, aggregation="collect")
        summary.summary("adafac_fisher_nonlinear/f_tilde/mean", jnp.mean(f_tilde))
        summary.summary("adafac_fisher_nonlinear/lambda", lam)
        return new_p
    ...
    def opt_fn(self, theta, is_training=False):
        ...
        class _Opt(opt_base.Optimizer):
            ...
            def init(self, params, model_state=None, num_steps=None, key=None):
                ...
                return AdaFisherNonlinearMLPLOptState(
            ...
            def update(self, opt_state, grad, loss, model_state=None, is_valid=False, key=None):
                ...
                next_opt_state = AdaFisherNonlinearMLPLOptState(
