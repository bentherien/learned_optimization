# coding=utf-8
# AdaFac + AdaFisher augmentation — Version: Everything
#
# Feature block: all Fisher features combined (raw factors, preconditioner,
# preconditioned gradient, per-channel momentum, Fisher×RMS cross, asymmetry,
# and Fisher-vs-RMS dominance).
...
class AdaFisherEverythingMLPLOptState:
    params:               Any
    state:                Any
...
@gin.configurable
class AdaFisherEverythingMLPLOpt(lopt_base.LearnedOptimizer):
    """
    AdaFac MLP learned optimizer — Version: Everything.

    All Fisher features appended to AdaFac feature vector:
        h_d                  raw col-norm² EMA                          (*, n_f)
        s_d                  raw row-norm² EMA                          (*, n_f)
        f_tilde              s_d * h_d + λ                              (*, n_f)
        rsqrt_f              1 / sqrt(f_tilde)                          (*, n_f)
        g * rsqrt_f          Fisher-preconditioned gradient             (*, n_f)
        m_fisher             per-channel preconditioned momentum        (*, n_m*n_f)
        rms_fisher_cross     rsqrt(rms) × rsqrt(f_tilde)               (*, n_rms*n_f)
        fisher_asymmetry     log(h_d) − log(s_d)                       (*, n_f)
        fisher_rms_dominance log(f_tilde) − log(rms[..., :1])          (*, 1)
    """
    ...
    def _mod(self, global_feat,
        ...
        # 4. Per-channel preconditioned momentum (outer product)
        m_exp     = m[..., :, None]                         # (*, n_m, 1)
        ...
        summary.summary("adafac_fisher_everything/avg_step_size", avg_step)
        summary.summary("adafac_fisher_everything/avg_step_size_hist", avg_step, aggregation="collect")
        summary.summary("adafac_fisher_everything/direction/mean_abs",  jnp.mean(jnp.abs(direction)))
        summary.summary("adafac_fisher_everything/magnitude/mean_abs",  jnp.mean(jnp.abs(magnitude)))
        summary.summary("adafac_fisher_everything/magnitude/mean",      jnp.mean(magnitude))
        summary.summary("adafac_fisher_everything/grad/mean_abs",       jnp.mean(jnp.abs(g)))
        summary.summary("adafac_fisher_everything/f_tilde/mean",        jnp.mean(f_tilde))
        summary.summary("adafac_fisher_everything/lambda",              lam)
    ...
    def opt_fn(self,
        ...
        class _Opt(opt_base.Optimizer):
            ...
            ) -> AdaFisherEverythingMLPLOptState:
                if num_steps is None:
                    raise ValueError("AdaFisherEverythingMLPLOpt requires num_steps at init time.")

                mom_roll, rms_roll, fac_vec_roll = self._get_rolling()
                n_f = len(parent._initial_fisher_decays)

                return AdaFisherEverythingMLPLOptState(
                    params=params,
                    state=model_state,
            ...
            def update(
                    self,
                    opt_state:   AdaFisherEverythingMLPLOptState,
                ...
                next_opt_state = AdaFisherEverythingMLPLOptState(
                    params=next_params,
                    state=model_state,
